...
def build_llm(provider: str = "auto", api_key: str = None, model: str = None):
    """Build an LLM instance from provider/key/model. Falls back to demo if no key."""
    _provider = provider.lower() if provider else "auto"

    # Explicit provider requested
    if _provider == "groq" or (_provider == "auto" and (api_key or os.getenv("GROQ_API_KEY"))):
        key = api_key or os.getenv("GROQ_API_KEY")
        if key:
            from langchain_groq import ChatGroq
            _model = model or "llama-3.3-70b-versatile"
            print(f"Using: Groq {_model}")
            return ChatGroq(model=_model, api_key=key, temperature=0), f"Groq {_model}"

    if _provider == "anthropic" or (_provider == "auto" and (api_key or os.getenv("ANTHROPIC_API_KEY"))):
        key = api_key or os.getenv("ANTHROPIC_API_KEY")
        if key:
            from langchain_anthropic import ChatAnthropic
            _model = model or "claude-sonnet-4-20250514"
            print(f"Using: Anthropic {_model}")
            return ChatAnthropic(model=_model, api_key=key, temperature=0), f"Claude Sonnet 4"

    if _provider == "google" or (_provider == "auto" and (api_key or os.getenv("GOOGLE_API_KEY"))):
        key = api_key or os.getenv("GOOGLE_API_KEY")
        if key:
            from langchain_google_genai import ChatGoogleGenerativeAI
            _model = model or "gemini-2.5-flash"
            print(f"Using: Google {_model}")
            return ChatGoogleGenerativeAI(model=_model, google_api_key=key, temperature=0), f"Gemini 2.5 Flash"

    if _provider == "openai" or (_provider == "auto" and (api_key or os.getenv("OPENAI_API_KEY"))):
        key = api_key or os.getenv("OPENAI_API_KEY")
        if key:
            from langchain_openai import ChatOpenAI
            _model = model or "gpt-4o-mini"
            print(f"Using: OpenAI {_model}")
            return ChatOpenAI(model=_model, api_key=key, temperature=0), f"GPT-4o Mini"

    print("No LLM API key found — demo mode")
    class _DemoLLM:
        def invoke(self, messages):
            from langchain_core.messages import AIMessage
            return AIMessage(content=(
                "⚠️ Demo Mode — No LLM API key configured.\n\n"
                "Set GROQ_API_KEY, ANTHROPIC_API_KEY, GOOGLE_API_KEY, or OPENAI_API_KEY "
                "in the sidebar or .env file.\n\nThe SQL engine and schema explorer are fully functional."
            ))
    return _DemoLLM(), "Demo mode (no key)"


# Module-level LLM (used when no per-request override is given)
llm, _llm_label = build_llm("auto")
print(f"Startup LLM: {_llm_label}")
...
def get_schema(db_path=None):
    """Returns the database schema string for the LLM system prompt."""
...
def run_query(sql_query, db_path=None):
    """Executes SQL against DuckDB. Returns (DataFrame, error_str)."""
    try:
...
def agent_workflow(user_question, chat_history=None, db_path=None, llm_override=None):
    """
    Self-Healing Loop + Memory + Simulation + Caching + Metrics.
    db_path and llm_override allow per-request runtime configuration.
    """
    if chat_history is None:
        chat_history = []

    active_llm = llm_override if llm_override is not None else llm
    ...
    if cached_sql:
        ...
        db_start = time.time()
        data, error = run_query(cached_sql, active_db)
    ...
    for attempt in range(3):
        ...
        db_start = time.time()
        data, error = run_query(sql_query, active_db)
        db_elapsed = time.time() - db_start
